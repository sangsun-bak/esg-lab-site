...
// ===== Utilities: ensure marked is available for Markdown =====
(function(){
  ...
  window.__ensureMarked = ensureMarked;
})();
...
async function fetchPubsEntries(){
  try{
    ...
    norm.sort((a,b)=> (b.date||'').localeCompare(a.date||''));
    return norm;
  }catch(e){ console.error('Failed to load pubs.json', e); return []; }
}
...
function renderPubsBoard(entries){
  ...
  entries.forEach(item => {
    ...
    if(item.volume || item.issue) bits.push(['Vol.', item.volume, 'No.', item.issue].filter(Boolean).join(' '));
    ...
    if(item.doi){
      const btn = document.createElement('a');
      btn.className='btn btn-sm';
      btn.href='https://doi.org/'+ item.doi.replace(/^https?:\/\/(dx\.)?doi\.org\//,'');
      btn.target='_blank';
      btn.textContent='DOI';
      actions.appendChild(btn);
    }
...
function openPubModal(item){
  ...
  if(!modal){
    modal = document.createElement('div'); modal.id='pubs-modal'; document.body.appendChild(modal);
  }
  modal.className='news-modal'; modal.innerHTML='';
  const overlay = document.createElement('div'); overlay.className='modal-overlay';
  const box = document.createElement('div'); box.className='modal-box';
  const title = document.createElement('h3'); title.textContent = item.title || '(무제)';
  const meta = document.createElement('div'); meta.className='muted';
  let bits = []; if(item.date) bits.push(item.date); if(item.authors) bits.push(item.authors);
  if(item.volume || item.issue) bits.push(['Vol.', item.volume, 'No.', item.issue].filter(Boolean).join(' '));
  if(item.pages) bits.push(item.pages);
  ...
  if(item.doi){
    ...
    btn.className='btn btn-sm';
    btn.href='https://doi.org/'+ item.doi.replace(/^https?:\/\/(dx\.)?doi\.org\//,'');
    btn.target='_blank'; btn.textContent='DOI 열기';
    actions.appendChild(btn);
  }
